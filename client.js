...
const igmp = require('./utils/igmp_client');
const keymap = {
...
ioHook.on('mousemove', event => {
  ...
  // console.log('x', x, 'y', y, screen);
  igmp.send(`/?type=mousemove&x=${x}&y=${y}&width=${1920||screen.width}&height=${1080||screen.height}`);
  // console.log('mousemove', Date.now(), '----')
});
ioHook.on('mousedown', event => {
  igmp.send(`/?type=mousedown&button=${['', 'left', 'right', 'middle'][event.button]}`);
});
// ioHook.on('mouseup', event => {
//   igmp.send(`/?type=mouseup&button=${['', 'left', 'right', 'middle'][event.button]}`);
// });
ioHook.on('mousewheel', event => {
  console.log(`/?type=mousewheel&x=0&y=${event.rotation*event.amount}`)
  igmp.send(`/?type=mousewheel&x=0&y=${event.rotation*event.amount}`);
});
ioHook.on('mousedrag', event => {
  // console.log(event)
  igmp.send(`/?type=mousedrag&x=${event.x}&y=${event.y}&width=${1920||screen.width}&height=${1080||screen.height}`);
});
ioHook.on('keydown', event => {
  ...
  console.log(`/?type=keydown&key=${keymap[event.rawcode] || ''}&modifier=${modifier.join(',')}`)
  igmp.send(`/?type=keydown&key=${keymap[event.rawcode] || ''}&modifier=${modifier.join(',')}`);
});
...
//   igmp.send(`/?type=keyup&key=${keymap[event.rawcode] || ''}&modifier=${modifier.join(',')}`);
// });
